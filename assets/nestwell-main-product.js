/**
 * Nestwell — Main Product section behavior
 * Originally inline in sections/nestwell-main-product.liquid, pulled out to
 * /assets so the browser can cache it instead of re-parsing it on every
 * product page load.
 *
 * Load with:
 *   <script src="{{ 'nestwell-main-product.js' | asset_url }}" defer></script>
 * (see the bottom of nestwell-main-product.liquid)
 */
document.addEventListener('DOMContentLoaded', function () {
  ...
  function activateMedia(mediaId) {
    thumbs.forEach(function (t) {
      t.classList.toggle('nestwell-product__thumb--active', t.getAttribute('data-media-id') === String(mediaId));
    });
  ...
  if (selector) {
    ...
    var optionGroups = selector.querySelectorAll('[data-option-position]');
    ...
    swatches.forEach(function (swatch) {
      swatch.addEventListener('click', function () {
        ...
        var group = selector.querySelector('[data-option-position="' + position + '"]');
        ...
        var label = selector.querySelector('[data-option-label="' + position + '"]');
  ...
  var toastContainer = document.querySelector('.nestwell-toast-container[data-section-id="' + sectionId + '"]');
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(window.routes ? window.routes.cart_add_url : '/cart/add.js', {
        ...
        .finally(function () {
          ...
          if (addToCartText && originalLabel) addToCartText.textContent = originalLabel;
        });
    });
  }
  ...
  var recWrap = root.querySelector('product-recommendations');
  if (recWrap) {
